...
public class SkillDash : SkillMain
{
    ...
    public override IEnumerator ISkillFunc(Controller attacker, bool mouseRotate = false)
    {
        ...
        PlayerAnimationListener animationListener = attacker.GetComponent<PlayerAnimationListener>();
        if (animationListener != null)
            animationListener.Attack_Dash();
        ...
        while (elapsedTime < lerpTime)
        {
            ...
            foreach (Collider c in colliders)
            {
                ...
                if (controller != null && 1 << controller.gameObject.layer == attacker.enemyLayer)
                {
                    ...
                    isStop = true;
                    AudioManager.Instance.Play2DSound("20121_Atk_Juro_HitB_01");
                }
            }
            if (isStop)
                break;
